...
const NativeAdCard = React.forwardRef<NativeAdCardRef, NativeAdCardProps>(
  ({ isImmersive = false, adData, onPress, onAdLoaded, onMediaLayout }, ref) => {
  ...
  const type = (adData?.type === 'LOCAL' || adData?.title || data?.getNextAd?.localAd) 
    ? 'LOCAL' 
  ...
  const rawAdData = localOverride || (adData?.title ? adData : data?.getNextAd?.localAd);
            ...
            {type === 'ADMOB' ? (
              <>
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => { 
                    setIsAdOptionsVisible(false); 
                    Linking.openURL('https://support.google.com/ads/contact/vio_other_info'); 
                  }}
                >
